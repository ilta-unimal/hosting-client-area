  ...
  <div class="mb-0 hover-scroll-x border-0">
    <div class="d-grid border-0">
        <ul class="nav nav-tabs flex-nowrap text-nowrap p-5 border-0 mb-0 pb-0">
            <li class="nav-item">
                <a class="nav-link btn btn-active-light btn-color-gray-600 btn-active-color-primary rounded-bottom-0 active" href="{{ route('client.iot-hosting.pending') }}">Pending</a>
            </li>
            <li class="nav-item">
                <a class="nav-link btn btn-active-light btn-color-gray-600 btn-active-color-primary rounded-bottom-0" href="{{ route('client.iot-hosting.active') }}">Active</a>
            </li>
        </ul>
    </div>
  </div>
            ...
            @else
                @foreach ($droplets as $item)
                    <tr>
                        ...
                        <td class="text-end">
                            <button class="btn btn-success btn-sm" data-bs-toggle="modal" data-bs-target="#approve{{ $item->id }}">
...
<div class="modal fade" tabindex="-1" id="approve{{$item->id}}">
  <div class="modal-dialog modal-dialog-centered">
      <div class="modal-content">
          <form action="{{ route('client.iot-hosting.approve', $item->id) }}" method="POST" id="update{{ $item->id }}">
            @csrf
            ...
            <div class="modal-body">
              <div class="mb-3">
                ...
                <input type="date" class="form-control form-control-solid" name="expired" value="{{ old('expired') }}" required/>
                @error('expired')
              ...
              <div class="mb-3">
                ...
                <input type="link" class="form-control form-control-solid" name="url" value="{{ old('url') }}" placeholder="Panel URL" required/>
                @error('url')
                <div class="text-sm text-danger">
                  {{ $message }}
                </div>
              ...
              <div class="mb-3">
                ...
                <input type="text" class="form-control form-control-solid" name="username" value="{{ old('username') }}" placeholder="Username" required/>
                @error('username')
                <div class="text-sm text-danger">
                  {{ $message }}
                </div>
              ...
              <div class="mb-3">
                ...
                <input type="text" class="form-control form-control-solid" name="password" value="{{ old('password') }}" placeholder="Password" required/>
                @error('password')
                <div class="text-sm text-danger">
                  {{ $message }}
                </div>
